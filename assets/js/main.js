...
(function () {
  ...
  /* ---------- "Inside the lab" stage + index gallery ----------
     Tabbed crossfade gallery with an auto-advance and scroll-linked drift.
     Deliberately self-contained: the site's [data-parallax] engine is gated
     by the hero on the homepage and is stopped long before this section
     scrolls in, so the gallery runs its own IntersectionObserver-gated rAF.
     Reduced-motion: no auto-advance, no drift, instant swaps. */
  (function () {
    var lab = document.getElementById("lab");
    if (!lab) return;
    var frames = [].slice.call(lab.querySelectorAll(".lab-frame"));
    var tabs = [].slice.call(lab.querySelectorAll(".lab-tab"));
    var ghost = lab.querySelector("[data-lab-ghost]");
    var caption = lab.querySelector("[data-lab-caption]");
    var metaEl = lab.querySelector("[data-lab-meta]");
    var descEl = lab.querySelector("[data-lab-desc]");
    var bar = lab.querySelector("[data-lab-progress]");
    var stage = lab.querySelector("[data-lab-stage]");
    if (!frames.length || frames.length !== tabs.length) return;

    var reduceMq = window.matchMedia("(prefers-reduced-motion: reduce)");
    var DURATION = 6500;
    var current = 0, elapsed = 0, lastT = 0;
    var visible = false, hovered = false, focused = false;
    var rafId = 0;

    function retick(el) {
      if (!el || reduceMq.matches) return;
    ...
    function select(i, fromUser) {
      ...
      if (i === current && fromUser !== "init") return;
      current = i;
      elapsed = 0;
      ...
      if (ghost) { ghost.textContent = "0" + (i + 1); retick(ghost); }
      if (metaEl) metaEl.textContent = tabs[i].getAttribute("data-meta") || "";
      if (descEl) descEl.textContent = tabs[i].getAttribute("data-desc") || "";
      retick(caption);
      if (fromUser === true && tabs[i] !== document.activeElement && focused) tabs[i].focus();
    }
    ...
    lab.addEventListener("mouseenter", function () { hovered = true; });
    lab.addEventListener("mouseleave", function () { hovered = false; });
    lab.addEventListener("focusin", function () { focused = true; });
    lab.addEventListener("focusout", function () { focused = false; });

    function paused() {
      return hovered || focused || document.hidden || reduceMq.matches;
    }
    ...
    function frame(t) {
      ...
      if (!paused()) {
        elapsed += dt;
        if (elapsed >= DURATION) { select(current + 1, false); }
        if (bar) bar.style.width = Math.min(100, (elapsed / DURATION) * 100).toFixed(2) + "%";
      }
      // scroll-linked drift: ghost slides sideways, stage breathes vertically
      if (!reduceMq.matches && window.innerWidth >= 1024 && stage) {
        var r = lab.getBoundingClientRect();
        var p = Math.max(-1, Math.min(1, (r.top + r.height / 2 - window.innerHeight / 2) / window.innerHeight));
        if (ghost) ghost.style.transform = "translate3d(" + (-p * 26).toFixed(2) + "px,-56%,0)";
        stage.style.transform = "translate3d(0," + (p * 12).toFixed(2) + "px,0)";
      }
      rafId = requestAnimationFrame(frame);
    }
    function start() { if (!rafId) { lastT = 0; rafId = requestAnimationFrame(frame); } }
    function stop() {
